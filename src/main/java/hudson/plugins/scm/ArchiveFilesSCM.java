...
/**
 * The Class ArchiveFilesSCM. ArchiveFilesSCM plugin for Jenkins supports checkout, extraction and also pooling of
 * archives files. The current version supports extraction of zip,tar,gz,jar,war,ear files. The type detection of
 * archive file is based on file name (i.e URL must end with zip,tar,tar.gz,jar,war,ear). Polling is done by checking
 * the last-modified header returned when connecting to a URL. If the type is unknown the plugin will simply copy the
 * file to workspace. Features
 * 
 * Supports Basic Authentication Supports Connection through Proxy Supports running on slave Supports http:// and
 * file:// portocols e.g - URL can be http://www.apache.org/dyn/closer.cgi/maven/binaries/apache-maven-3.0.4-bin.tar.gz
 * file:///C:/Arjun/felix.jar (On Windows) file:///home/arjun/felix.jar (On Unix/Linux)
 */
...
public class ArchiveFilesSCM extends hudson.scm.SCM {
	...
	/** The Constant LOGGER. */
	private static final Logger LOGGER = Logger.getLogger(ArchiveFilesSCM.class.getName());
	 ...
	 */
	public ArchiveFilesSCM(String[] yourls, boolean clear, String[] username, String[] password) {
	 ...
	 * 
	 * @see hudson.scm.SCM#checkout(hudson.model.AbstractBuild, hudson.Launcher, hudson.FilePath,
	 *      hudson.model.BuildListener, java.io.File)
	 */
	@Override
	public boolean checkout(AbstractBuild<?, ?> build, Launcher launcher, FilePath workspace, BuildListener listener,
			...
			try {
				...
				if (proxyConfiguration == null) {
					...
					listener.getLogger().println(
							"Proxy is configured" + "\n" + ProxyConfiguration.getXmlFile().asString());
					// Proxy should be used when it is configured in Jenkins global configuration
					connection = url.openConnection(proxyConfiguration.createProxy());
					// setting authentication parameters for proxy. Authenticator class is not used here because of two
					// reasons. The actual URL to download file may be secured and we can not set two Authenticators at
					// the same time using Authenticator.setDefault and the second reason is username/password caching
					// bug in JDK.
					// (say if user gives incorrect username/password and when validation is performed, that
					...
					// User name and password is not cached when it is set as request parameter.)
					// http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6626700
					if (proxyUserName != null) {
						listener.getLogger().println("User Name and Password is configured to connect through proxy");
						connection.setRequestProperty(
								"Proxy-Authorization",
								"Basic "
										+ new String(Base64.encodeBase64((proxyUserName + ":" + proxyPassword)
												.getBytes())));
					}
				}
				...
				// Saving last modified time stamp for later use while polling for source code change
				...
				// creating a timestamp file which will be used to see if source file is updated since last download
				FilePath timestamp = workspace.child("." + fileName + "-timestamp");

				if (!workspace.exists()) {
					workspace.mkdirs();
				}
				if (timestamp.exists() && sourceLastUpdatedTimestamp == timestamp.lastModified()) {
					listener.getLogger().println("File is up to date");
				} else {
					// for HTTP downloads, enable automatic retry for added resilience
					is = new CountingInputStream(url.getProtocol().equals("http") ? new RetryableHttpStream(url)
							: connection.getInputStream());

					if (url.toExternalForm().endsWith(".zip") || url.toExternalForm().endsWith(".jar")
							|| url.toExternalForm().endsWith(".war") || url.toExternalForm().endsWith(".ear")) {
						listener.getLogger().println("Compression type is zip/jar/war");
						...
						listener.getLogger().println("Compression type unknow. Hence directly downloding the file");
						workspace.child(fileName).copyFrom(is);
					}
					listener.getLogger().println("Downloaded " + urlString + " to " + workspace.toURI());
		...
		// Adding LastModificationDateAction to build for later use while pooling. This is optimized code as
		// calcRevisionsFromBuild method will not be invoked
		build.addAction(action);
		listener.getLogger().println(
				"Total time taken to download files in millis: " + (System.currentTimeMillis() - start));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see hudson.scm.SCM#compareRemoteRevisionWith(hudson.model.AbstractProject, hudson.Launcher, hudson.FilePath,
	 * hudson.model.TaskListener, hudson.scm.SCMRevisionState)
	 */
	@Override
	protected PollingResult compareRemoteRevisionWith(AbstractProject<?, ?> project, Launcher launcher,
			FilePath workspace, TaskListener listener, SCMRevisionState baseline) throws IOException,
			InterruptedException {
		LOGGER.log(ALL, "compareRemoteRevisionWith() Enter >>>");
		PollingResult pollingResult = PollingResult.NO_CHANGES;

		AbstractBuild<?, ?> lastBuild = project.getLastBuild();
		// When no previous build exits and this is the first build we got to build
		if (lastBuild != null) {
		...
		LastModifiedDateAction action = lastBuild.getAction(LastModifiedDateAction.class);
		if (action == null) {
			listener.getLogger().println("There are significant changes.\ncompareRemoteRevisionWith() Exit >>>");
			LOGGER.log(ALL, "compareRemoteRevisionWith() Exit >>>");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see hudson.scm.SCM#calcRevisionsFromBuild(hudson.model.AbstractBuild, hudson.Launcher,
	 * hudson.model.TaskListener)
	 */
	@Override
	public SCMRevisionState calcRevisionsFromBuild(AbstractBuild<?, ?> arg0, Launcher arg1, TaskListener arg2)
	 ...
	 */
	@Extension
	public static final class ArchiveFilesSCMDescriptorImpl extends SCMDescriptor<ArchiveFilesSCM> {
		...
		public SCM newInstance(StaplerRequest req, JSONObject formData) throws FormException {
			...
			String usernames[] = req.getParameterValues("archive_files_scm_username");
			LOGGER.log(INFO, "URL :" + Arrays.deepToString(urls) + "\n User Name :" + Arrays.deepToString(usernames));
			LOGGER.log(ALL, "newInstance() Exit >>>");
			return new ArchiveFilesSCM(urls, req.getParameter("archive_files_scm_clear") != null, usernames,
		 ...
		 */
		public FormValidation doRequiredCheck(@QueryParameter final String value) throws ServletException {
	...
	public static final class URLTuple {
		...
		public URLTuple(String urlString, String username, String password) {
			...
			// In the url is not secured initialize user name and password with empty strings
